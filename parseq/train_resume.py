...
def main(config: DictConfig):
    ...
    #     pretrained_dict = get_pretrained_weights(config.pretrained)
    #     model_dict = model.state_dict()
    #     # 1. filter out unnecessary keys
    #     pretrained_dict = {k: v for k, v in pretrained_dict.items() if k not in ['head.weight', 'head.bias', 'text_embed.embedding.weight']}
    ...
    if config.pretrained is None:
        raise InvalidModelError(f"No pretrained weights")
    
    print(f'Load checkpoint from: {config.pretrained}')
    ...
    checkpoint = ModelCheckpoint(monitor='val_NED', mode='max', save_top_k=10, save_last=True,
