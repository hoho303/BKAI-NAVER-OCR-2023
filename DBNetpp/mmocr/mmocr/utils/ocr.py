...
class MMOCR:
    ...
    def single_inference(self, model, arrays, batch_mode, batch_size=0):
        ...
        result = []
        fnames = self.args.filenames
        log_file = open('/workspace/DBNetpp/mmocr/inference-result/log.txt', 'w')
        out_folder = '/workspace/DBNetpp/mmocr/inference-result/json'
        print(self.args.img)
        ...
        if batch_mode:
            if batch_size == 0:
                ...
                fname_chunks = [
                    fnames[i: i + n] for i in range(0, len(fnames), n)
                ]

                for idx, (arr_chunk, fname_chunk) in enumerate(zip(arr_chunks, fname_chunks)):
                    if (idx + 1) % 100 == 0:
                        log_file.write(f'{idx + 1}/{len(arr_chunks)}\n')
                        log_file.flush()
                        print(f'{idx + 1}/{len(arr_chunks)}')

                    result = inference(model, arr_chunk, batch_mode=True)
                    for fname, pred in zip(fname_chunk, result):
                        with open(os.path.join(out_folder, fname + '.json'), 'w', encoding='utf-8') as wf:
                            json.dump(pred, wf, indent=3, ensure_ascii=False)
        else:
            for idx, arr in enumerate(arrays):
                result.append(inference(model, arr, batch_mode=False))
                if (idx + 1) % 100 == 0:
                    log_file.write(f'{idx + 1}/{len(arrays)}\n')
                    log_file.flush()
                    print(f'{idx + 1}/{len(arrays)}')
        return result
